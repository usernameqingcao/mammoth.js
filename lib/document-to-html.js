...
function DocumentConverter(options) {
    return {
        convertToHtml: function(element) {//解析入口
            var comments = _.indexBy(
                element.type === documents.types.document ? element.comments : [],
                "commentId"
            );
            var conversion = new DocumentConversion(options, comments);
            return conversion.convertToHtml(element);//调用转换为html代码
        }
    };
}
...
function DocumentConversion(options, comments) {
    ...
    function convertParagraph(element, messages, options) {
     ...
     function htmlPathForParagraph(element, messages) {
        let style =findStyle(element);
        if (style) {
           //新增h标签修改  把原有的return style.to改为以下
            if (element.styleId) {
                messages.push(unrecognisedStyleWarning("paragraph", element));
            }
            else if(style.to._elements.length===1){//返回单独标签
                return htmlPaths.topLevelElement(style.to._elements[0].tagName, converStyleForaAtributes(element))
            }else if(style.to._elements.length===2){//ul-li标签 ol-li标签
                style.to._elements[1]= htmlPaths.topLevelElement(style.to._elements[1].tagName, converStyleForaAtributes(element))._elements[0]
                return style.to
            }
            else{
                return style.to;
            ...
            // 新增 把原先 defaultParagraphStyle 改为生成p标签并且添加 attributes 属性
...
function converStyleForaAtributes(element, options) {//新增一个方法生成 style 内容
    let styleMap = []
    if (!options) {
      options = {}
    }
